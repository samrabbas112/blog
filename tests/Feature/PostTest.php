...
class PostTest extends TestCase
{
    ...
    public function it_should_store_the_post()
    {
        ...
        $data = [
            ...
            'category_id' => $category->id ,// Ensure this category exists in the database or use a factory to create it.
            'admin_id' => $this->admin->id, // Ensure this admin exists or create one using a factory.
            'status' => 'published',
            'published_at' => now(),
            'meta_description' => 'Sample meta description',
            'meta_keywords' => 'sample,post,keywords',
            'flag' => 'top'
        ];
        $response = $this->withHeaders([
            'Authorization' => 'Bearer ' . $this->token,
            'X-Requested-With' => 'XMLHttpRequest', // This header indicates an AJAX request
        ])->json('POST', route('api.v1.posts.store',$data));
